...
from model import PrototypicalNet, DistanceCalculator
import numpy as np
...
def TA_eval(args, device, N_query, model, model_):
	...
	prediction_results = predict(args, device, model, test_loader, model_)
	write_csv(prediction_results, args.testcase_pred_csv)
	if args.mode != 'test': eval(args)

def MY_eval(prediction_results, valid_digits_labels):
	prediction_results = torch.tensor(prediction_results).cuda()
	hit = 0
	for prediction in prediction_results:
		hit += torch.sum(prediction == valid_digits_labels).item()
	total = prediction_results.view(-1).shape[0]
	print('Accuracy: {:.2f} %'.format(hit / total * 100))
	print()

def calculate_distance(args, support_feature, q_feature, DNet):
	...
	elif args.distance_type == 'Parametric_function':
		distance = DNet(support_feature, q_feature.unsqueeze(0))
	return distance

def convert2predictions(args, support_feature, query_feature, model_):
    ...
    for q_feature in query_feature:
        for feature in q_feature:
            distance = calculate_distance(args, support_feature, feature, model_)
            predictions.append(torch.argmax(distance, dim = 1).item())
    return predictions

def convert2distances(args, support_feature, query_feature, DNet):
	distances = []
	support_feature = support_feature.view(-1, args.n_shot, support_feature.shape[-1])
	support_feature = torch.mean(support_feature, dim = 1).squeeze(1)
	for q_feature in query_feature:
		distance = calculate_distance(args, support_feature, q_feature, DNet)
...
def train(args, device):
	...
	if args.mode == 'train':
		...
		if args.distance_type == 'Parametric_function':
			DNet = DistanceCalculator()
			print(DNet)
		...
		if args.load != -1:
			...
			if args.distance_type == 'Parametric_function':
				DNet.load_state_dict(torch.load(save_path + str(args.load) + '_.ckpt'))

		optimizer = optim.SGD(filter(lambda param : param.requires_grad, list(list(PNet.parameters()) + list(DNet.parameters())) if args.distance_type == 'Parametric_function' else PNet.parameters()), lr = 1e-3, weight_decay = 0.012, momentum = 0.9)
		...
		for epoch in range(args.load + 1, args.epochs):
			...
			for index, (support_image, query_image, label) in enumerate(tqdm(train_data, ncols = 70)):
				...
				support_feature = PNet(batch_support_image)
				query_feature = PNet(batch_query_image)
				distances = convert2distances(args, support_feature, query_feature, DNet)
			...
			with torch.no_grad():
				...
				for index, (support_image, query_image, label) in enumerate(tqdm(valid_data, ncols = 70)):
					...
					query_feature = PNet(batch_query_image)
					distances = convert2distances(args, support_feature, query_feature, DNet)
					prediction_results.append(convert2predictions(args, support_feature, query_feature, DNet))
				...
				if avg_loss < best_loss:
					...
					if args.distance_type == 'Parametric_function':
						torch.save(DNet.state_dict(), save_path + 'best_.ckpt', _use_new_zipfile_serialization = False)
				torch.save(PNet.state_dict(), save_path + '{}.ckpt'.format(epoch), _use_new_zipfile_serialization = False)
				if args.distance_type == 'Parametric_function':
					torch.save(DNet.state_dict(), save_path + '{}_.ckpt'.format(epoch), _use_new_zipfile_serialization = False)

				if args.n_shot == 1:
					TA_eval(args = args, device = device, N_query = args.n_query, model = PNet, model_ = DNet)
				else:
					MY_eval(prediction_results, valid_digits_labels)
...
def test(args, device):
	...
	PNet.load_state_dict(torch.load('./Problem1/test.ckpt'))
	PNet.eval()
	TA_eval(args = args, device = device, N_query = args.n_query, model = PNet, model_ = None)
