...
def calculate_distance(args, support_feature, query_feature, model_):
    if args.distance_type == 'Euclidean_distance':
        ...
        distance = model_(support_feature, query_feature.unsqueeze(0))
    return distance
...
def predict(args, device, model, data_loader, model_):
    prediction_results = []
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(tqdm(data_loader, ncols = 70)):
            ...
            prediction_results.append(convert2predictions(args, support_feature, query_feature, model_))
